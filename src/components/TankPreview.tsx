...
/* ------------------------------------------------------------------ */
/*  SVG sub-components for plants & rocks                             */
/* ------------------------------------------------------------------ */
...
function JavaFern() {
  return (
    ...
    >
      <path
        d="M45 130 Q43 90 35 55 Q30 30 34 15 Q37 4 45 2 Q53 4 56 15 Q60 30 55 55 Q47 90 45 130Z"
        fill="#2d6a4f"
        opacity=".88"
      />
      <path
        d="M38 115 Q26 75 20 48 Q17 32 22 22 Q27 16 32 22 Q35 38 38 115Z"
        fill="#40916c"
        opacity=".75"
      />
      <path
        d="M52 115 Q64 75 70 48 Q73 32 68 22 Q63 16 58 22 Q55 38 52 115Z"
        fill="#40916c"
        opacity=".75"
      />
      <path
        d="M42 120 Q30 85 25 60 Q23 50 28 42 Q32 38 35 44 Q36 55 42 120Z"
        fill="#52b788"
        opacity=".5"
      />
    </svg>
...
function Anubias() {
  return (
    ...
    >
      ...
      <ellipse
        cx="50" cy="44" rx="20" ry="14"
        fill="#1a4d2e" opacity=".88"
        transform="rotate(-8,50,44)"
      />
      <ellipse
        cx="30" cy="34" rx="17" ry="12"
        fill="#2d6a4f" opacity=".82"
        transform="rotate(12,30,34)"
      />
      <ellipse
        cx="70" cy="30" rx="17" ry="11"
        fill="#2d6a4f" opacity=".82"
...
function SeiryuStone() {
  return (
    ...
    >
      <polygon
        points="12,80 6,45 22,18 42,5 60,12 78,28 84,55 72,80"
        fill="#6b7d8e"
        opacity=".92"
      />
      <polygon
        points="18,74 12,48 28,22 44,14 56,20 66,38 60,74"
        fill="#7d8fa0"
        opacity=".45"
      />
...
function DragonStone() {
  return (
    ...
    >
      <path
        d="M10,65 Q4,42 14,26 Q24,10 40,8 Q56,6 65,22 Q72,38 68,58 Q64,65 10,65Z"
        fill="#8B6914"
        opacity=".9"
      />
...
export default function TankPreview({ state }: TankPreviewProps) {
  ...
  useEffect(() => {
    ...
    const ctx = gsap.context(() => {
      gsap.from(Array.from(rocksRef.current!.children), {
        y: 30,
        opacity: 0,
        duration: 0.55,
        stagger: 0.1,
  ...
  useEffect(() => {
    ...
    const ctx = gsap.context(() => {
      gsap.from(Array.from(plantsRef.current!.children), {
        opacity: 0,
        scale: 0.8,
        duration: 0.55,
        stagger: 0.1,
  ...
  return (
    <div className="glass-panel-strong rounded-2xl p-6 shadow-sm">
      ...
      {/* Scene backdrop */}
      <div className="flex items-end justify-center overflow-hidden rounded-xl bg-gradient-to-b from-mist/50 via-aqua-light/20 to-sage/20 p-8 pb-12">
        ...
        <div
          ...
          style={{
            ...
            border: "4px solid rgba(180,200,210,0.55)",
            boxShadow:
              "0 8px 32px rgba(0,60,40,0.12), inset 0 0 40px rgba(255,255,255,0.08)",
          }}
        >
          {/* Water gradient */}
          <div
            ...
            style={{
              background:
                "linear-gradient(180deg, rgba(173,232,244,0.35) 0%, rgba(144,224,239,0.25) 40%, rgba(45,106,79,0.30) 100%)",
            }}
          />
          ...
          <div
            ...
            style={{
              background: `linear-gradient(180deg, ${LIGHTING_OVERLAY[state.lighting]} 0%, transparent 60%)`,
            }}
          />
          ...
          <div
            className="absolute inset-0 transition-colors duration-700"
            style={{
              background: `linear-gradient(155deg, ${LIGHTING_RAYS[state.lighting]} 0%, transparent 40%, transparent 60%, ${LIGHTING_RAYS[state.lighting]} 100%)`,
            }}
          />
          ...
          <div
            ...
            style={{
              height: "14%",
              background:
                "linear-gradient(180deg, #d4a574 0%, #c9975e 100%)",
              borderTop: "1px solid rgba(180,140,90,0.3)",
            }}
          />
      ...
      {(state.plants.length > 0 || state.rocks.length > 0) && (
        <div className="mt-6 flex flex-wrap gap-2">
          ...
          {state.rocks.map((r) => (
            <span
              key={r}
              className="rounded-full bg-forest/10 px-3 py-1 text-xs font-medium text-forest/70"
            >
              {r}
            </span>
          ))}
